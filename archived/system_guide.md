...
### Cơ chế ho�### 6. Màn hình đăng nhập (Greetd + Tuigreet)
Hệ thống sử dụng `greetd` và `tuigreet` thay cho SDDM để loại bỏ hoàn toàn X11, giúp tiết kiệm pin tuyệt đối (Card NVIDIA được tắt hoàn toàn ở màn hình đăng nhập).
...
**2. Cấu hình tự động gọi Hyprland:**
Sửa file `/etc/greetd/config.toml`:
```toml
[terminal]
vt = 1
[default_session]
...
**3. Cấu hình mở khóa GNOME Keyring:**
Thêm 2 dòng sau vào cuối file `/etc/pam.d/greetd` để Chrome/Cốc Cốc tự động lấy được mật khẩu đã lưu:
```text
...
### 7. Quản lý Năng lượng & Tối ưu Pin (TLP)
Hệ thống sử dụng TLP để tự động ngủ đông các khe PCI, Wifi, SSD... giúp tiết kiệm năng lượng tối đa, ngang ngửa Windows.

**1. Cài đặt:**
*(Lưu ý: Không cài `intel-undervolt` cho chip Gen 12th)*
```bash
yay -S tlp tlp-rdw
```
...
**3. Cấu hình chống Lag Chuột và Lách tách Loa:**
Mở file `/etc/tlp.conf` và sửa lại/bỏ dấu `#` ở các dòng sau:
```text
# Tắt tự động ngủ đông USB (Chống delay chuột/phím)
USB_AUTOSUSPEND=0

# Tắt tiết kiệm pin Card âm thanh (Chống tiếng lách tách)
SOUND_POWER_SAVE_ON_AC=0
SOUND_POWER_SAVE_ON_BAT=0
```
Sau đó nạp lại cấu hình bằng lệnh: `sudo tlp start`

### 8. Tối ưu Swaync (Chống giật/lag toàn hệ thống)
Mặc định `swaync` thỉnh thoảng bám vào card NVIDIA và hiệu ứng Blur gây ra khựng (stutter) toàn màn hình khi có thông báo. Lỗi này bắt nguồn từ việc SwayNC dùng công nghệ Vulkan để vẽ đồ họa.

**1. Cài đặt Driver Vulkan cho Intel:**
Bắt buộc phải có để các App dùng Vulkan (như SwayNC) không tự động đánh thức card NVIDIA:
```bash
yay -S vulkan-intel
```

**2. Tắt Blur cho thông báo:**
Mở `~/.config/hypr/hyprland.conf`, tìm và comment (`#`) 2 cụm lệnh áp dụng blur cho swaync, hoặc chuyển thành `blur = off`:
```text
# name = swaync-blur
...
**3. Ép Swaync chạy trên Card Tích hợp (Intel):**
Tạo file ghi đè dịch vụ Systemd để truyền biến môi trường cực mạnh ép dùng EGL và Vulkan của Intel:
```bash
mkdir -p ~/.config/systemd/user/swaync.service.d/

cat << 'INNER_EOF' > ~/.config/systemd/user/swaync.service.d/override.conf
[Service]
...
### 9. Công cụ Giám sát Phần cứng (Monitoring Tools)
- **`btop` / `htop`**: Quản lý CPU, RAM, Tiến trình (cài bằng `yay -S btop`).
- **`nvtop`**: Công cụ đỉnh cao giám sát đồng thời cả Card Intel và NVIDIA. Cho biết chính xác App nào đang dùng Card nào và tiêu thụ bao nhiêu Watt. (`yay -S nvtop`)
- **`powertop`**: Chẩn đoán mức tiêu thụ Watt tổng của Laptop (Chỉ xem được tab Overview khi **rút sạc**). (`yay -S powertop`)
- **`nvidia-smi`**: Bảng điều khiển riêng của NVIDIA. **CẢNH BÁO:** KHÔNG DÙNG lệnh `watch -n 1 nvidia-smi` liên tục vì nó sẽ liên tục "đánh thức" card NVIDIA mỗi giây, khiến nó ngốn 5W-6W không báo giờ ngủ.

### 10. Sự thật về Tiêu thụ Năng lượng (Hybrid Laptop)
Rất nhiều người lầm tưởng rằng "Tổng máy báo 20W nghĩa là CPU đang ngốn 15W". Thực tế trên Laptop Gaming, 20W nhàn rỗi (idle) được chia ra:
- **Màn hình (Backlight):** ~3W - 5W (tùy độ sáng).
- **Ổ cứng SSD NVMe Gen 4:** ~1.5W - 2W.
- **RAM & Card Wifi:** ~2W.
- **NVIDIA (Nếu bị đánh thức bởi watch hoặc Chrome):** ~5W - 6W.

👉 **Suy ra:** Con chip Intel H-Series (hiệu năng cao) thực chất chỉ tiêu thụ khoảng **4W - 6W** lúc rảnh rỗi. Đây là một con số **cực kỳ lý tưởng**.

**Làm sao để tối ưu thêm?**
1. Đảm bảo rút sạc để đo mức ăn điện chính xác bằng `powertop`.
2. Tắt "Hardware Acceleration" trong Google Chrome để tránh nó vơ vét card NVIDIA.
3. Để máy yên tĩnh 1 phút, nếu `powertop` báo tổng máy rớt xuống 10W - 13W thì cỗ máy của bạn đã được tối ưu đạt đến "Cảnh giới tối cao" của Arch Linux!
